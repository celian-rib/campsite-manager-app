...
public class Toaster {
	/**
	 * Permet de lancer l'affichage d'un toast avec sa durée et son fadein/out time Il faut lui
	 * passer en paramètre deux Panes. Le premier, celui sur lequel le toast sera mis, et un autre,
	 ...
	 */
	public static void playTransition(Pane toast, ToastBasket basket, int durationMillis,
			int fadeinoutMillis) {
		...
		FadeTransition fadeTransitionOut =
				new FadeTransition(Duration.millis(fadeinoutMillis), toast);
		...
		SequentialTransition seqTransition = new SequentialTransition(fadeTransition,
				new PauseTransition(Duration.millis(durationMillis)), fadeTransitionOut);
		seqTransition.play();
	}
	 ...
	 * @see ToastBuilder.playTransition(Pane toast, Pane container, int durationMillis, int
	 *      fadeinoutMillis)
	...
	public static BorderPane createToast(ToastType type, String content) {
		...
		pane.setStyle("-fx-background-radius: 5; -fx-background-color: " + rgbaBackground
				+ "; -fx-padding: 15px;");
		pane.setOpacity(1);
